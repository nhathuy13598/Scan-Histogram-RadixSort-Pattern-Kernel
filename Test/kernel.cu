	...
	int nBits, int * blockSizes)
{
	...
	for (int bit = 0; bit < sizeof(uint32_t) * 8; bit += nBits)
	{
		...
		dim3 gridSize((nBins - 1) / blockSizes[1] + 1);
		...
		// Cap phat bo nho
		int *d_in; CHECK(cudaMalloc(&d_in, nBins * sizeof(int)));
		int *d_out; CHECK(cudaMalloc(&d_out, nBins * sizeof(int)));
		int *d_blkSums; CHECK(cudaMalloc(&d_blkSums, gridSize.x * sizeof(int)));

		// Sao chep du lieu tu host sang device
		CHECK(cudaMemcpy(d_in, hist, nBins * sizeof(int), cudaMemcpyHostToDevice));

		// Goi ham kernel scan
		scanBlkKernel << <gridSize, blockSizes[1], smem_size >> > (d_in, nBins, d_out, d_blkSums);
		...
		scanSumKernel << <gridSize, blockSizes[1] >> > (d_out, d_blkSums, nBins);
		CHECK(cudaGetLastError());

		// Chep du lieu tu device sang host
		CHECK(cudaMemcpy(histScan, d_out, nBins * sizeof(int), cudaMemcpyDeviceToHost));
